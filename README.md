...
1. Login stores token and cached user data in local storage.
2. Navbar tries backend user fetch first, then falls back to cached or token-derived user data.
3. Join requests from Home are stored in local storage and reflected in My Account.
4. Created projects, hackathons, and jobs from My Account are stored in local storage and shown on Home.
5. Bio editing is currently local frontend state.
...
- Connect join requests to backend APIs (like we would have made it if we had time)
